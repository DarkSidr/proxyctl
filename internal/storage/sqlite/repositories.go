...
func (r *inboundRepository) Create(ctx context.Context, inbound domain.Inbound) (domain.Inbound, error) {
	...
	_, err := r.db.ExecContext(
		ctx,
		`INSERT INTO inbounds (id, type, engine, node_id, domain, port, tls_enabled, transport, path, sni, enabled, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		inbound.ID,
		inbound.Type,
		inbound.Engine,
...
func (r *inboundRepository) List(ctx context.Context) ([]domain.Inbound, error) {
	rows, err := r.db.QueryContext(ctx, `SELECT id, type, engine, node_id, domain, port, tls_enabled, transport, path, sni, enabled, created_at FROM inbounds ORDER BY created_at ASC, id ASC`)
	if err != nil {
