...
После каждой завершённой задачи **всегда выполняй релиз без лишних вопросов**:
1. `git add <изменённые файлы>`
2. Коммит с внятным сообщением (тип: описание)
3. Новый тег `v0.1.NNN` (следующий после последнего — проверяй через `git tag | sort -V | tail -3`)
4. `git push origin main --tags`

Не спрашивай разрешения — просто делай коммит + тег + пуш после каждой задачи.
